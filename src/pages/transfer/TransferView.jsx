...
console.log(accountNumber);
  const handleSubmit = (e) => {
    ...
    let url = "";
    if (type === 'simple') {
      url = "http://localhost/api/v1/transfer/simple";
    } else if (type === 'auto') {
      url = "http://localhost/api/v1/autoTransfer/register";
    }
    ...
    <div className="px-24">
      ...
      <main className="flex flex-col items-center flex-grow p-4">
        <div className="text-5xl font-noto font-semibold mb-10">
          ...
          님께 <span className="text-hanaGreen">{formData.amount}</span>원을 이체합니다.
        </div>
          ...
          <div className="bg-white font-noto text-3xl rounded-lg shadow-lg p-6 mt-6">
            <div className="space-y-8">
              ...
              <div className="flex justify-between">
                ...
                <span>{formData.recipientNumber}</span>
                {type === 'auto' && (
                <span>{formData.toAccountNumber}</span>
              )}
              </div>
              <div className="flex justify-between">
                <span className="font-hana2 text-gray-400 text-4xl">이체금액</span>
